...
Структура файлів:
  ...
  stats_regression.py  — множинна лінійна регресія
  plots_regression.py  — графіки для регресії / візуалізації
  stats_pca.py         — МГК (метод головних компонент)
  plots_pca.py         — графіки для МГК
  tab_pca.py           — вкладка МГК
"""
...
class TabLoad(ttk.Frame):
    ...
    def _load(self):
        ...
        # Оновити списки колонок у вкладках, що їх потребують
        for tab in self.app.notebook.tabs():
            widget = self.app.notebook.nametowidget(tab)
            if isinstance(widget, (TabRegression, TabPCA)):
...
# ══════════════════════════════════════════════════════════════
#  ВКЛАДКА 5 — МНОЖИННА ЛІНІЙНА РЕГРЕСІЯ
# ══════════════════════════════════════════════════════════════
class TabRegression(ttk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(ctrl, text="  Залежна змінна:", bg=C['bg'],
                 fg=C['text']).pack(side='left', padx=(20, 5))
        ...
        SectionHeader(self.result_frame, "  Діагностична діаграма  ",
                      accent=True).pack(fill='x', padx=4, pady=(8, 2))
    ...
    def _run(self):
        ...
        DataTable(self.result_frame,
                  columns=list(res['coef_table'][0].keys()),
                  data=res['coef_table']).pack(fill='both', expand=True, pady=4)

        info = (f"R² = {res['R2']}  |  R²_adj = {res['R2_adj']}  |  "
                f"F = {res['F_stat']} (критичне {res['F_crit']}) → "
                f"{'ЗНАЧУЩА' if res['model_sig'] else 'НЕЗНАЧУЩА'}")
        tk.Label(self.result_frame, text=info,
                 bg=C['panel'],
                 fg=C['green'] if res['model_sig'] else C['accent2'],
                 font=('Consolas', 10, 'bold')).pack(pady=4)

        tk.Label(self.result_frame,
                 text=f"Довірчий інтервал для σ²: [{res['sigma2_ci_lo']}; {res['sigma2_ci_hi']}]",
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tab_defs = [
            ...
            ("  📐  Множинна регресія  ",   TabRegression),
            ("  📊  Візуалізація  ",        TabVisualization),
            ("  🔄  МГК  ",                TabPCA),          # ← НОВИЙ пункт 5
        ]
        ...
        self.status.info(
            "Оберіть файл даних на вкладці «Дані» та натисніть «Завантажити»")
