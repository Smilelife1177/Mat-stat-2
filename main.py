...
class TabVisualization(ttk.Frame):
    ...
    def _build(self):
        ...
        # Notebook для чотирьох графіків
        nb = ttk.Notebook(self)
        nb.pack(fill='both', expand=True, padx=8, pady=8)
        ...
        glyph_ctrl.pack(fill='x')
        tk.Label(glyph_ctrl, text="Макс. спостережень:",
                 bg=C['bg'], fg=C['text'],
                 font=('Consolas', 9)).pack(side='left')
        self.glyph_max_var = tk.StringVar(value="40")
        ttk.Spinbox(glyph_ctrl, from_=5, to=200, increment=5,
                    textvariable=self.glyph_max_var,
                    width=6, state='readonly').pack(side='left', padx=(6, 0))
        tk.Label(glyph_ctrl,
                 text="  (кожна зірка = одне спостереження; промінь = нормована ознака [0–1])",
                 bg=C['bg'], fg=C['subtext'],
                 font=('Consolas', 8)).pack(side='left', padx=(10, 0))

        self.glyph_panel = PlotPanel(self.tab_glyph)
    ...
    def _run(self):
        ...
        try:
            max_obs = int(self.glyph_max_var.get())
        except ValueError:
            max_obs = 40
        self.glyph_panel.show(pr.glyph_star_figure(self.app.df, max_obs))

        self.app.status.ok("Візуалізація побудована (паралельні координати, теплова карта, бульбашкова діаграма, гліфи)")
